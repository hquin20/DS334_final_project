...
- change improvement option from Improve to Worsen with radio buttons - observe the opposite effects
...
server <- function(input, output, session) {
  ...
  pivot_longer(c(Anxiety, Depression, Insomnia, OCD), 
      ...
      labs(x = "Music Effect",
           y = "Proportion",
           title = glue::glue("\n\nFigure 2. Comparing Effects of Listening to ",
                              input$sel_genre, " ", 
                              input$sel_often))
  })
  ...
  output$sum_table <- renderDataTable({
    ...
    wrangled_data |> select(-c(Timestamp, Classical, Country, EDM, Folk, Gospel, `Hip Hop`,
                               ...
                               music_improve))  ## add text so we have a title for this table 
  })
...
potentially add: line plot that compares worsen to improve to no effect of each: never, some, frequently, etc. 
...
- change variable names so they are cleaner OR add descriptions of each variable as text in the shiny app
